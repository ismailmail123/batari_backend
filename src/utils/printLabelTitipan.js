...
// Dynamic import untuk printer yang compatible
let printer;
try {
    ...
    console.warn('Printer module not available, printing disabled');
    printer = null;
}
...
async function printLabelTitipan({
    ...
    try {
        ...
        // ===== BORDER UTAMA =====
        currentY += 18;
        doc.rect(3, currentY, 220.77, 190)
            .strokeColor('#000000')
            .lineWidth(1)
            .stroke();
        ...
        doc.font("Helvetica-Bold")
            .fontSize(7)
            .text("• BARANG TITIPAN HARAP DIAMBIL MAX 7 HARI •", 5, currentY, {
                align: "center",
                width: 216.77
            });
        ...
        // Cek jika printer available
        if (!printer) {
            console.log('✅ PDF label titipan berhasil dibuat (printing disabled)');
            console.log('📄 File PDF:', tempFilePath);
            return;
        }

        // Print berdasarkan platform
        if (os.platform() === 'win32') {
            // Windows
            ...
            const thermalPrinter = printers.find(p =>
                p.name.includes('POS') || p.name.includes('80mm') || p.name.includes('thermal')
            );
            ...
            if (thermalPrinter) {
                printer.printFile({
                    ...
                    success: function(jobID) {
                        console.log("✅ Label titipan berhasil dicetak! Job ID:", jobID);
                    },
                    error: function(err) {
                        console.error("❌ Gagal mencetak label titipan:", err);
                    }
                });
            } else {
                console.log('🖨️  Printer thermal tidak ditemukan, menggunakan printer default');
                printer.printFile({
                    filename: tempFilePath,
                    success: function(jobID) {
                        console.log("✅ Label titipan berhasil dicetak! Job ID:", jobID);
        ...
        // Hapus file sementara setelah beberapa detik
        setTimeout(() => {
            if (fs.existsSync(tempFilePath)) {
                fs.unlinkSync(tempFilePath);
            }
        }, 10000); // 10 detik untuk memastikan print selesai
